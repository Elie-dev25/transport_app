...
def admin_or_responsable(view):
    ...
    Permet de distinguer qui fait quoi dans les logs.
    """
    @wraps(view)
    def decorated_function(*args, **kwargs):
...
def superviseur_access(view):
    ...
    Maintient la traçabilité du rôle exact
    """
    @wraps(view)
    def decorated_function(*args, **kwargs):
        # Vérifier l'authentification
        ...
        from app.utils.audit_logger import log_user_action
        log_user_action('CONSULTATION', view.__name__, f"Accès lecture seule")
